...
class ControlMeanModel(nn.Module):
    """常数预测模型：预测值恒为 Control 组细胞的全局均值"""
    def __init__(self, control_hvg_mean, control_emb_mean):
        super().__init__()
        self.control_hvg_mean = torch.tensor(control_hvg_mean, dtype=torch.float32)
        self.control_emb_mean = torch.tensor(control_emb_mean, dtype=torch.float32)

    def forward(self, ctrl_gene, ctrl_expr, drug1_smiles, drug2_smiles):
        """
        接口和真实模型完全一致，但忽略所有输入，只返回常数
        返回: pred_embs [B, S, emb_dim], pred_hvgs [B, S, 2000]
        """
        B = len(drug1_smiles)
        S = ctrl_gene.shape[1]
        ...
        pred_embs = self.control_emb_mean.to(device).unsqueeze(0).unsqueeze(0).expand(B, S, -1)
        pred_hvgs = self.control_hvg_mean.to(device).unsqueeze(0).unsqueeze(0).expand(B, S, -1)
...
def main():
    ...
    # Step 1: 用训练集计算 Control 组细胞的全局均值
    print(f"\n{'='*80}")
    print(f"📦 计算 Control 组细胞全局均值 (Control Baseline)")
    print(f"{'='*80}")
    ...
    for batch in train_loader:
        # 这里取 Control 组（未扰动）的 HVG
        control_hvg = batch["control_hvg_vectors"][0]   # [S, 2000]
        all_hvg.append(control_hvg.mean(dim=0).numpy())
        # emb 暂时用 perturb emb 的均值作为替代
        perturb_emb = batch["perturb_embeddings"][0]
        all_emb.append(perturb_emb.mean(dim=0).numpy())

    control_hvg_mean = np.mean(all_hvg, axis=0)  # [2000]
    control_emb_mean = np.mean(all_emb, axis=0)   # [emb_dim]

    print(f"  训练集扰动条件数: {len(all_hvg)}")
    print(f"  Control HVG 均值 L2 norm: {np.linalg.norm(control_hvg_mean):.3f}")
    print(f"  Control Emb 均值 L2 norm: {np.linalg.norm(control_emb_mean):.3f}")

    # 创建常数预测模型
    model = ControlMeanModel(control_hvg_mean, control_emb_mean)
    ...
    # 汇总表格
    print(f"\n{'='*100}")
    print(f"📊 CONTROL BASELINE FINAL SUMMARY")
    print(f"{'='*100}")
    ...
    print(f"\n✅ Control Baseline Evaluation Complete!")
