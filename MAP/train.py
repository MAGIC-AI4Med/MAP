...
def parse_args():
    parser = argparse.ArgumentParser(description="Training MAP model.")
    ...
    # model
    parser.add_argument("--smile_encoder", type=str, default='MAP-KG')
...
def main():
    ...
    ds_train = TahoePerturbDatasetSE(
    ...
    if rank == 0:
        ...
        ds_external_test = TahoePerturbDatasetSE(
            cell_lines=args.cell_lines,
            split="external_test",
            base_dir=args.data_base_dir,
            se_inputs_base_dir=args.se_inputs_base_dir,
        ...
        print(f"✅ External test set: {len(ds_external_test)} samples")
    ...
    for epoch in range(args.epochs):
        ...
        if rank == 0 and external_loader is not None:
            print("\n" + "="*80)
            print(f"[Eval @ epoch start] epoch={epoch+1}")
            print("="*80)
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, disable=(rank != 0))):
            ...
            if rank == 0 and external_loader is not None and args.eval_every_steps > 0:
                if (global_step + 1) % args.eval_every_steps == 0:
                    model.eval()
                    print("\n" + "="*80)
                    print(f"[Eval mid] epoch={epoch+1}, step={global_step}")
                    print("="*80)
