# =========================
# File: prnet_eval_utils.py
# Evaluation utilities for PRnet model (aligned with ours_6)
#
# Metrics for HVG:
...
def evaluate_PRnet(
    ...
    """
    Full evaluation function for PRnet (aligned with ours_6 eval_full.py).
    ...
    Args:
        model: PRnet model (PGM module)
        val_loader: Validation DataLoader (sequential mode)
    ...
    for comb_idx in tqdm(range(total_combinations), disable=not verbose):
        for sample_idx in range(num_samples_per_comb):
            ...
            # Extract data: shape is [set_size, G] (no batch dim in sequential mode)
            control_hvg = batch_single["control_hvg_vectors"].to(device)  # [set_size, G]
            target_hvg = batch_single["perturb_hvg_vectors"].to(device)  # [set_size, G]

            # Encode drug dynamically (some datasets don't pre-encode)
            if "encoded_drug" in batch_single:
                encode_label = batch_single["encoded_drug"].to(device)  # [set_size, 1024]
            else:
                # Compute drug encoding on the fly (for NIPS/Sciplex datasets)
                from rdkit import Chem
                from rdkit.Chem import AllChem
                smiles = batch_single["drug_smiles"]
                dose = batch_single["drug_conc"]
                set_size = control_hvg.shape[0]

                # Ensure SMILES is string
                ...
                if mol is None:
                    ...
                    fp = AllChem.GetMorganFingerprintAsBitVect(mol, 2, useFeatures=True, nBits=1024)
                    encoded_single = np.array(list(fp), dtype=np.float32)
                    safe_dose = max(float(dose), 0.0)
                    encoded_single = encoded_single * np.log10(safe_dose + 1)

                encode_label = torch.from_numpy(np.tile(encoded_single, (set_size, 1))).to(device)

            # Build noise
            set_size = control_hvg.shape[0]
            noise = torch.randn(set_size, 10, device=device)

            # Forward pass: model returns [set_size, 2*G] (mean + var concatenated)
            gene_reconstructions = model(control_hvg, encode_label, noise)

            # Split to get only mean (first G dimensions)
            dim = gene_reconstructions.size(1) // 2
            pred_mean = gene_reconstructions[:, :dim]  # [set_size, G]

            # Compute pseudobulk means (average over set_size)
            pred_hvg_mean_np = pred_mean.mean(dim=0).float().cpu().numpy()  # [G]
            true_hvg_mean_np = target_hvg.mean(dim=0).float().cpu().numpy()  # [G]
            control_hvg_mean_np = control_hvg.mean(dim=0).float().cpu().numpy()  # [G]
...
def _print_results_as_full_table(per_cell_line_results, overall_avg, de_topk, num_samples_per_comb):
    ...
    print("\n" + "=" * 110)
    print(f"PRnet FULL EVALUATION RESULTS (de_topk={de_topk}, samples_per_comb={num_samples_per_comb})")
    print("=" * 110)
    print()
