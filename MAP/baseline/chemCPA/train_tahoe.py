...
- Same 6 cell lines for both train and test
...
# Reuse the dataset class from ds_tahoe_multi_cell_se
from ds_tahoe_multi_cell_se import TahoePerturbDatasetSE
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train chemCPA on Tahoe dataset (aligned with ours_6)"
    )

    # Dataset arguments (same as ours_6/train.sh)
    parser.add_argument("--data_base_dir", type=str,
                        default="/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/ours_3/preprocessed",
                        help="Base directory for preprocessed Tahoe data")
    parser.add_argument("--se_inputs_base_dir", type=str,
                        default="/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/ours_4/preprocessed_se_inputs_memmap",
                        help="Base directory for SE input memmaps")
    parser.add_argument("--cell_lines", type=str, nargs="+",
                        default=["CVCL_0023", "CVCL_0480", "CVCL_0069", "CVCL_0131", "CVCL_1098", "CVCL_1056"],
                        help="Cell lines (ALL 6, same for train AND test!)")
    ...
    parser.add_argument("--save_dir", type=str, default="./outputs_chemCPA_ours6",
    ...
    # WandB
    parser.add_argument("--wandb_project", type=str, default="chemCPA-tahoe-ours6-aligned",
...
class ChemCPAModule(L.LightningModule):
    ...
    def __init__(self, hparams, canonical_smiles, num_genes, num_cell_lines, val_loader=None, smiles_to_idx=None, eval_freq_steps=10000):
        super().__init__()
        ...
        try:
            ...
            # Compute ECFP4 fingerprints on-the-fly using rdkit
        ...
        self.model = ComPert(
            ...
            num_covariates=[num_cell_lines],  # one covariate (cell line)
            device="cuda" if torch.cuda.is_available() else "cpu",
            hparams=hparams,
    ...
    def _common_step(self, batch, batch_idx):
        ...
        covariates = batch["covariates"]  # list of [batch_size * set_size, num_cell_lines]
    ...
    def training_step(self, batch, batch_idx):
        ...
        # For Tahoe we only have one covariate (cell line one-hot)
        # but ComPert expects a list of covariates, so we pass it directly


        # 1. Forward pass with latent_basal
        # predict returns: normalized_reconstructions, cell_drug_embedding, (latent_basal, drug_embedding, latent_treated)
        ...
        # Periodic evaluation (every eval_freq_steps steps)
...
def tahoe_collate_fn(batch, smiles_to_idx):
    """Collate function - converts TahoePerturbDatasetSE batch to chemCPA format.
    ...
    covariates = [torch.stack(cell_line_ohe_list, dim=0)]  # [batch_size * set_size, num_cell_lines]
    ...
    return {
        ...
        "covariates": covariates,       # cell line one-hot
    }
...
def main():
    ...
    # Print configuration
    print("\n" + "=" * 80)
    print("ChemCPA Training (aligned with ours_6)")
    print("=" * 80)
    ...
    # First, collect all unique SMILES across ALL cell lines AND BOTH splits!
    ...
    # This handles any edge cases where canonicalization differs
    for cell_line in args.cell_lines:
        cell_line_dir = os.path.join(args.data_base_dir, cell_line)
        meta_path = os.path.join(cell_line_dir, f"{cell_line}_meta.pkl")
        with open(meta_path, "rb") as f:
    ...
    # Build datasets - SAME cell lines for train AND test (different splits!)
    ...
    print("\n🏗️ Building validation dataset (split='external_test')...")
    val_dataset = TahoePerturbDatasetSE(
        cell_lines=args.cell_lines,
        split="external_test",
        base_dir=args.data_base_dir,
        se_inputs_base_dir=args.se_inputs_base_dir,
    ...
    train_loader = DataLoader(
        ...
        collate_fn=lambda b: tahoe_collate_fn(b, smiles_to_idx),
        pin_memory=True,
    ...
    # Create model (pass val_loader and smiles_to_idx for periodic evaluation)
    print("\n🤖 Creating ChemCPA model...")
    model = ChemCPAModule(
        hparams=hparams,
        canonical_smiles=canonical_smiles,
        num_genes=args.num_genes,
        num_cell_lines=len(args.cell_lines),
        val_loader=val_loader,
        smiles_to_idx=smiles_to_idx,
        eval_freq_steps=args.eval_freq_steps,
    )
    print(f"✅ Model created with {model.num_drugs} drugs, {model.num_genes} genes, {model.num_cell_lines} cell lines")
