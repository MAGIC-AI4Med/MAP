...
- Same 3 cell lines for both train and test
...
# Sciplex dataset class
from ds_sciplex_lora_se import SciplexPerturbDatasetSE
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train chemCPA on Sciplex dataset (aligned with ours_6)"
    )

    # Dataset arguments (same as ours_6/train_sciplex.py)
    parser.add_argument("--data_base_dir", type=str,
                        default="/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/external_dataset_sciplex/preprocessed",
                        help="Base directory for preprocessed Sciplex data")
    parser.add_argument("--se_inputs_base_dir", type=str,
                        default="/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/external_dataset_sciplex/preprocessed_se_inputs_memmap/sciplex",
                        help="Base directory for SE input memmaps")
    parser.add_argument("--cell_lines", type=str, nargs="+",
                        default=["A549", "K562", "MCF7"],
                        help="Cell lines (ALL 3, same for train AND test!)")
    ...
    parser.add_argument("--save_dir", type=str, default="./outputs_chemCPA_sciplex",
    ...
    # WandB
    parser.add_argument("--wandb_project", type=str, default="chemCPA-sciplex-ours6-aligned",
...
def sciplex_collate_fn(batch, smiles_to_idx):
    """Collate function - converts SciplexPerturbDatasetSE batch to chemCPA format.

    Args:
        batch: list of samples from SciplexPerturbDatasetSE
...
def main():
    ...
    # Print configuration
    print("\n" + "=" * 80)
    print("ChemCPA Training (Sciplex, aligned with ours_6)")
    print("=" * 80)
    ...
    for cell_line in args.cell_lines:
        ...
        # Deduplicate within cell line to avoid redundant rdkit work
        unique_in_cell = set(smi for smi in meta["drug_smiles"] if smi is not None)
        print(f"    Found {len(unique_in_cell)} unique SMILES")

        # Apply the SAME canonicalization as SciplexPerturbDatasetSE does!
    ...
    train_dataset = SciplexPerturbDatasetSE(
    ...
    val_dataset = SciplexPerturbDatasetSE(
        cell_lines=args.cell_lines,
        split="test",
        base_dir=args.data_base_dir,
        se_inputs_base_dir=args.se_inputs_base_dir,
    ...
    train_loader = DataLoader(
        ...
        collate_fn=lambda b: sciplex_collate_fn(b, smiles_to_idx),
        pin_memory=True,
