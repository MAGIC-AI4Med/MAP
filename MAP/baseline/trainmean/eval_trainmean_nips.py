# eval_trainmean_nips.py
"""
TrainMean Baseline Evaluation for NIPS dataset
...
3. Supports both per cell type and per drug evaluation modes
...
from data.ds_nips_lora_se import NIPSPerturbDatasetSE
...
def compute_trainmean_vector(internal_loader, device):
    ...
    """
    print("\n" + "="*80)
    print("📊 Computing TrainMean Vector from Train Set (NIPS)")
    print("="*80)
...
def evaluate_per_cellline(
    ...
    """
    Evaluate TrainMean baseline per cell type

    Uses the exact same metrics as eval.py.
    """
    # Store data per cell type
    cell_type_data = defaultdict(lambda: {
        'hvg_loss': [],
        'pred_hvg_means': [],
        'true_hvg_means': [],
        'control_hvg_means': [],
    ...
    for batch_data in tqdm(test_loader, desc="Evaluating TrainMean (per cell type)"):
        hvgs = batch_data["perturb_hvg_vectors"].to(device)
        ...
        for i in range(B):
            ...
            cl_data = cell_type_data[cell_line]
    ...
    # Compute metrics for each cell type
    ...
    for cell_type, data in cell_type_data.items():
        ...
        per_cell_type_results[cell_type] = results

    # Compute overall average
    overall_avg = _compute_overall_average(per_cell_type_results)

    if verbose:
        _print_results_per_celltype(per_cell_type_results, overall_avg, de_topk)
...
def evaluate_per_drug(
    ...
    for batch_data in tqdm(test_loader, desc="Evaluating TrainMean (per drug)"):
        ...
        for i in range(B):
            ...
            d_data['cell_types'].add(cell_line)
    ...
    for smile, data in drug_data.items():
        ...
        results['num_cell_types'] = len(data['cell_types'])
...
def _compute_overall_average(per_group_results):
    """Compute overall average across all groups (cell types or drugs)"""
    if not per_group_results:
        return {}

    all_metrics = list(next(iter(per_group_results.values())).keys())
    # Exclude non-numeric fields from average
    exclude_fields = ['cell_type', 'cell_type', 'smiles', 'num_cell_types', 'drug_names', 'cell_types', 'drug_names']
...
def _print_results_per_celltype(per_cell_type_results, overall_avg, de_topk):
    """Print results table for per cell type evaluation"""
    if not per_cell_type_results:
        print("No results to display.")
        return

    cell_types = sorted(per_cell_type_results.keys())
    ...
    print("\n" + "="*110)
    print("📊 TRAINMEAN BASELINE - PER CELL TYPE RESULTS (NIPS)")
    print("="*110)
    ...
    for metric_name, metric_key, fmt in metrics:
        ...
        for ct in cell_types:
            value = per_cell_type_results[ct][metric_key]
    ...
    print("="*110)
    print(f"\n📈 Summary:")
    print(f"   Number of cell types: {len(per_cell_type_results)}")
    print(f"   Total samples evaluated: {overall_avg['total_samples']}")
    print(f"   Average samples per cell type: {overall_avg['total_samples'] / len(per_cell_type_results):.1f}")
    print()
...
def _print_results_per_drug(per_drug_results, overall_avg, de_topk):
    ...
    print("\n" + "="*160)
    print("📊 TRAINMEAN BASELINE - PER DRUG RESULTS (NIPS)")
    print("="*160)
    ...
    for smile in sorted_smiles:
        ...
        row = (
            f"{drug_name_short:<30} {smiles_short:<40} {results['num_cell_types']:>8} {results['num_samples']:>10} "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="TrainMean Baseline Evaluation for NIPS dataset",
    ...
    parser.add_argument("--se_inputs_base_dir", type=str,
                       default="/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/external_dataset_nips/preprocessed_se_inputs",
                       help="Base directory for precomputed SE inputs (memmap)")
    parser.add_argument("--cell_types", "--cell_lines", type=str, nargs='+',
                       default=['TcellsCD4', 'NKcells', 'Myeloidcells', 'Bcells', 'TcellsCD8', 'Tregulatorycells'],
                       help="Cell types to evaluate on")
    ...
    parser.add_argument("--mode", type=str, choices=['cellline', 'drug', 'both'], default='both',
                       help="Evaluation mode: per cell type, per drug, or both")
...
def main():
    ...
    internal_dataset = NIPSPerturbDatasetSE(
    ...
    external_dataset = NIPSPerturbDatasetSE(
        cell_lines=args.cell_types,
        split="test",
        base_dir=args.data_base_dir,
        se_inputs_base_dir=args.se_inputs_base_dir,
    ...
    if args.mode in ['cellline', 'both']:
        ...
        print("✅ Per cell type evaluation completed!\n")
    ...
    if args.mode == 'both':
        print("="*80)
        print("📋 FINAL SUMMARY - TrainMean Baseline (NIPS)")
        print("="*80)
