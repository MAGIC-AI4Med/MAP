#!/usr/bin/env python
"""
Build heterogeneous graph data for XPert HG pre-training on Sciplex dataset.
...
def load_genes_from_h5ad(h5ad_path: str, embedding_path: str, ensembl_to_symbol):
    """Load gene symbols from Sciplex h5ad and get ESM embeddings
    Sciplex gene ids are Ensembl ids (with version) - need to map to HGNC symbol
    """
    print(f"Loading h5ad from {h5ad_path}")
    adata = sc.read_h5ad(h5ad_path, backed='r')

    # Get gene ids from var - column is named 'id' for Sciplex
    if "id" not in adata.var.columns:
        raise KeyError("h5ad does not have 'id' in var columns")

    ensembl_ids = list(adata.var["id"].astype(str).values)
    print(f"[Sciplex] Genes in dataset: {len(ensembl_ids)}")

    # Load ESM embeddings dict (keys are HGNC symbols)
    ...
    # Map Ensembl id (strip version) to HGNC symbol
    valid_genes = []
    valid_symbols = []
    emb_list = []
    missing_map = 0
    ...
    for ensembl_id in ensembl_ids:
        # Strip version number - everything after '.'
        ensembl_base = ensembl_id.split('.')[0]
        if ensembl_base not in ensembl_to_symbol:
            missing_map += 1
            continue
        symbol = ensembl_to_symbol[ensembl_base]
        if symbol in gene_emb_dict:
            valid_symbols.append(symbol)
            emb = gene_emb_dict[symbol].numpy()
            emb_list.append(emb)
            valid_genes.append(ensembl_id)  # keep original id for indexing
        else:
            missing_emb += 1

    gene_emb_matrix = np.stack(emb_list, axis=0)
    gene_to_idx = {symbol: idx for idx, symbol in enumerate(valid_symbols)}

    print(f"  Missing Ensembl mapping: {missing_map}, missing EMB embedding: {missing_emb}")
    print(f"  Final valid genes: {len(valid_symbols)}, gene embedding shape: {gene_emb_matrix.shape}")

    return valid_symbols, gene_to_idx, gene_emb_matrix
...
def load_unique_drugs_from_h5ad(h5ad_path: str):
    """Extract unique drugs with SMILES from Sciplex h5ad"""
...
def build_hgnc_mapping(hgnc_path: str):
    """Build mappings from HGNC:XXXX to gene symbol and from Ensembl id to HGNC symbol"""
    ...
    for _, row in df.iterrows():
        ...
        # Map Ensembl id (without version) to symbol
        if pd.notna(row['ensembl_gene_id']):
            ensembl_id = str(row['ensembl_gene_id']).split('.')[0]
            ensembl_to_symbol[ensembl_id] = symbol

    print(f"Built mapping: {len(hgnc_to_symbol)} HGNC entries, {len(ensembl_to_symbol)} Ensembl mappings")
    return hgnc_to_symbol, ensembl_to_symbol
...
def main():
    ...
    sciplex_h5ad = "/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/external_dataset_sciplex/raw/sciplex3_pp_hvgenes_scFM_resplit_filtered_ESM2.h5ad"
    ...
    # 0. Build HGNC mapping - get both HGNC to symbol and Ensembl to symbol
    hgnc_to_symbol, ensembl_to_symbol = build_hgnc_mapping(hgnc_path)

    # 1. Load genes from h5ad - map Ensembl to HGNC symbols, get ESM embeddings
    # Keep full 5120 dimension
    gene_list, gene_to_idx, gene_emb_matrix = load_genes_from_h5ad(sciplex_h5ad, gene_emb_path, ensembl_to_symbol)
    num_genes = len(gene_list)
    out_gene_feat = os.path.join(output_dir, f"sciplex_all_gene_node_feat_{num_genes}.npy")
    np.save(out_gene_feat, gene_emb_matrix)
    print(f"Saved gene node features to {out_gene_feat}")

    # Save gene list
    with open(os.path.join(output_dir, f"sciplex_gene_list_{num_genes}.json"), 'w') as f:
        json.dump(gene_list, f, indent=2)

    # 2. Load drugs from h5ad
    drug_list, drug_names, drug_smiles, drug_to_idx = load_unique_drugs_from_h5ad(sciplex_h5ad)
    num_drugs = len(drug_list)

    # Save mappings
    drug_smiles_map = {idx: smi for idx, (name, smi) in enumerate(drug_list)}
    with open(os.path.join(output_dir, f"sciplex_drugs_smiles_{num_drugs}.json"), 'w') as f:
        json.dump(drug_smiles_map, f, indent=2)
    drug_name_map = {idx: name for idx, (name, smi) in enumerate(drug_list)}
    with open(os.path.join(output_dir, f"sciplex_drugs_names_{num_drugs}.json"), 'w') as f:
    ...
    ppi_out = os.path.join(output_dir, f"sciplex_PPI_edge_index_all_{ppi_edge_index.shape[1]}_pairs_weighted.npy")
    ...
    dti_out = os.path.join(output_dir, f"sciplex_DTI_edge_index_all_{dti_edge_index.shape[1]}_pairs.npy")
    ...
    dds_out = os.path.join(output_dir, f"sciplex_DDS_edge_index_all_{dds_edge_index.shape[1]}_pairs_weighted.npy")
    ...
    with open(os.path.join(output_dir, f"sciplex_mappings.json"), 'w') as f:
        json.dump(mappings, f, indent=2)

    print("\n" + "="*60)
    print("SCIPLEX HG PREPROCESSING SUMMARY:")
    print("="*60)
