...
# ✅ 改动 1：从 NIPS 版本的脚本导入 build_crisp_dataloader
# 你需要确保 nips_crisp_multicell.py 在 PYTHONPATH 或同目录
from ds_nips_crisp import build_crisp_dataloader
...
class CRISPMultiCellConfig:
    # =========================
    # NIPS data paths (✅ 改动 2：替换 Tahoe 的 base_dir/UC/cell_lines 等字段)
    # =========================
    h5ad_path: str
    shards_dir: str
    hvg_root: str
    ...
    # NIPS 的“cell line”语义是 cell_type（字符串列表）
    cell_types: List[str]
    ...
    # Output
    # =========================
    out_dir: str = "./checkpoints_crisp_multicell_nips"
    exp_name: str = "nips_multicell_run1"
...
class CRISPMultiCellTrainer:
    def __init__(self, cfg: CRISPMultiCellConfig):
        ...
        print("\n" + "=" * 80)
        print("Initializing CRISP Multi-Cell Trainer (NIPS)")
        print("=" * 80)

        # ✅ 改动 3：build_crisp_dataloader 参数换成 NIPS 版本
        (
            self.train_loader,
            self.num_genes,
            self.fm_dim,
...
if __name__ == "__main__":
    ...
    cfg = CRISPMultiCellConfig(
        ...
        # smiles embedding
        drug_emb_pt_path="/mnt/petrelfs/fengjinghao/CRAFT/VirtualCell/baselines/CRISP/data/drug_embeddings/rdkit2d_smiles_embeddings_NIPS.pt",

        # NIPS cell types（示例；请替换为你实际想训练的 cell_type 列表）
        cell_types=[
            "T cells CD4+",
            "NK cells",
            "Myeloid cells",
        ...
        device="cuda",
        out_dir="./checkpoints_crisp_multicell_nips",
