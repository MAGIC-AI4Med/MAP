# =========================
# File: train_nips.py
# Training script for NIPS dataset - follows the same DDP training logic as train.py
# =========================
import os
import time
import argparse
...
def parse_args():
    parser = argparse.ArgumentParser(description="Training on NIPS dataset with DDP.")
    ...
    parser.add_argument("--cell_types", "--cell_lines", type=str, nargs='+', required=True,
                        help="Cell types to train on (NIPS uses cell_types)")
...
def main():
    ...
    # 训练数据集
    if rank == 0:
        print(f"📊 Loading NIPS training dataset...")

    ds_train = NIPSPerturbDatasetSE(
    ...
    if rank == 0:
        print("📊 Loading NIPS test dataset...")
        ds_external_test = NIPSPerturbDatasetSE(
            cell_lines=args.cell_types,
            split="test",
            base_dir=args.data_base_dir,
            se_inputs_base_dir=args.se_inputs_base_dir,
        ...
        print(f"✅ NIPS test set: {len(ds_external_test)} unique (cell_type, drug) combinations")
    ...
    for epoch in range(args.epochs):
        if rank == 0:
            print(f"\n{'='*80}")
            print(f"Epoch {epoch+1}/{args.epochs} [NIPS]")
            print(f"{'='*80}")

        # ===== Epoch开始时的evaluation =====
        if rank == 0 and external_loader is not None:
            print("\n" + "="*80)
            print(f"[Eval @ epoch start] epoch={epoch+1} [NIPS]")
            print("="*80)
            ...
            model.eval()
            t0 = time.time()
            # Evaluation always in float32 to avoid NaN issues with AMP bf16
            with autocast(dtype=amp_dtype, enabled=False):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, disable=(rank != 0))):
            ...
            # print('ctrl_gene:', ctrl_gene.shape, ctrl_gene.max(), ctrl_gene.min())
            # print('ctrl_expr:', ctrl_expr.shape, ctrl_expr.max(), ctrl_expr.min())
            # print('tgt_emb:', tgt_emb.shape, tgt_emb.max(), tgt_emb.min())
            # print('hvgs:', hvgs.shape, hvgs.max(), hvgs.min())
            # print('smiles:', smiles)
            # exit()
            ...
            with ctx:
                with autocast(dtype=amp_dtype, enabled=use_amp):
                    ...
                    # print('pred_embs:', pred_embs.shape, pred_embs.max(), pred_embs.min())
                    # print('pred_hvgs:', pred_hvgs.shape, pred_hvgs.max(), pred_hvgs.min())
            ...
            if rank == 0 and external_loader is not None and args.eval_every_steps > 0:
                if (global_step + 1) % args.eval_every_steps == 0:
                    model.eval()
                    print("\n" + "="*80)
                    print(f"[Eval mid] epoch={epoch+1}, step={global_step} [NIPS]")
                    print("="*80)
        ...
        if rank == 0 and accum_count > 0:
            print(f"\n[Epoch {epoch+1}] Avg Train Loss [NIPS]:")
            print(f"  Total: {loss_accum/accum_count:.4f}")
            print(f"  Emb:   {emb_loss_accum/accum_count:.4f}")
            print(f"  HVG:   {hvg_loss_accum/accum_count:.4f}")
            loss_accum = emb_loss_accum = hvg_loss_accum = 0.0
